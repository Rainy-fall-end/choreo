...
ffmpeg_path='/root/ffmpeg-git-20240301-amd64-static/ffmpeg' # point to your specific copy of the ffmpeg binary
...
def animate_stick(seq, ghost=None, ghost_shift=0, edge_types=None, edge_opacities=None, threshold=0, edge_class=None, figsize=None, zcolor=None, pointer=None, ax_lims=(-0.4,0.4), speed=45,
                  dot_size=20, dot_alpha=0.5, lw=2.5, cmap='cool_r', pointer_color='black', cloud=False, cloud_alpha=0.03, skeleton=True, skeleton_alpha=0.3,name = ""):
    if zcolor is None:
    ...
    ax.text(1, 5, 7, name, color='red', fontsize=12, ha='right', va='top')
    ...
    if edge_types is not None:
        ...
        dot_color = "black"
        pts = ax.scatter(seq[0,:,0],seq[0,:,1],seq[0,:,2], c=dot_color, s=dot_size, cmap='viridis', alpha=dot_alpha)
    print(max(seq[0,:,0]))
    print(max(seq[0,:,1]))
    print(max(seq[0,:,2]))
    ...
    if ax_lims:
        ax.set_xlim(np.min(seq[:,:,0]),np.max(seq[:,:,0]))
        ax.set_ylim(np.min(seq[:,:,1]),np.max(seq[:,:,1]))
        ax.set_zlim(np.min(seq[:,:,2]),np.max(seq[:,:,2]))
    plt.savefig("/root/autodl-tmp/2.jpg")
    plt.close(fig)
